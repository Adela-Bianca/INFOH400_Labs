...
public class DicomInstanceServices {
    ...
    public boolean sendInstanceToSCP(String host, int port, String calledAET){
        try {
            ...
            String sopInstanceUID = al.get(TagFromName.SOPInstanceUID).getDelimitedStringValuesOrEmptyString();
            String sopClassUID = al.get(TagFromName.SOPClassUID).getDelimitedStringValuesOrEmptyString();
            new StorageSOPClassSCU(host,
                    port, 
                    calledAET,
                    "HISSCU", // Name for the server
                    instanceFile.toString(),
                    sopClassUID,
                    sopInstanceUID,
                    0); //Compression level
